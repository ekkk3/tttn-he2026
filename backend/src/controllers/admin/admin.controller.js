...
export const listUsers = asyncHandler(async (req, res) => {
  const rows = await query(
    "SELECT id, full_name, email, phone, role, is_active FROM users WHERE is_deleted = 0 ORDER BY id DESC"
  );
  res.json({ users: rows });
});
...
export const userOrders = asyncHandler(async (req, res) => {
  const rows = await query('SELECT * FROM orders WHERE user_id = ? ORDER BY id DESC', [req.params.user]);
...
export const updateUser = asyncHandler(async (req, res) => {
  ...
  await query(
    `UPDATE users SET full_name = COALESCE(?, full_name), phone = COALESCE(?, phone),
       role = COALESCE(?, role), is_active = COALESCE(?, is_active) WHERE id = ?`,
    [full_name, phone, role, is_active, req.params.user]
  );
  res.json({ message: 'Da cap nhat nguoi dung.' });
});
export const destroyUser = asyncHandler(async (req, res) => {
  await query('UPDATE users SET is_deleted = 1 WHERE id = ?', [req.params.user]);
